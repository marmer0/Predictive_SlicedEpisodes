...
library(GGally)
##STARTING EDA 
pts_smooth <- function(data, mapping, ...) {
  ggplot(data = data, mapping = mapping) +
    geom_point(size = 2) +
    geom_smooth(method = "loess", span = 1.1, color = "darkblue", alpha = 0.2, ...)
}
...
train %>% 
  select(rain_tomorrow, min_temp, max_temp, rainfall, wind_gust_speed,wind_speed3pm, humidity3pm, pressure3pm, temp3pm, cloud3pm) %>% 
  ggpairs(columns = 2:10, aes(color = rain_tomorrow, alpha = 0.75),
          lower = list(continuous = pts_smooth),
